...
class Transforms:
    def __init__(self, size, s=1.0, mean=None, std=None, blur=False):
        self.train_transform = [
            ...
            # torchvision.transforms.RandomApply([MaskPixels(p=0.2)], p=0.5),
            torchvision.transforms.RandomHorizontalFlip(),
            # torchvision.transforms.RandomVerticalFlip(),
            # torchvision.transforms.RandomCrop(size-4),
            ...
            # MaskPixels(p=0.3)
            # MaskBands(p=0.3)
            # torchvision.transforms.RandomApply([MaskPixels(p=0.2)], p=0.5),  # p = 0.3 works well
            # torchvision.transforms.RandomApply([MaskBands(p=0.2)], p=0.5),
            # torchvision.transforms.RandomApply([ShufflePixel()], p=0.5),  # # no work
            # torchvision.transforms.RandomApply([torchvision.transforms.ColorJitter(0.8 * s, 0.8 * s, 0.8 * s, 0.2 * s)],
            #                                    p=0.8),
            # torchvision.transforms.RandomCrop(size=11),
            # torchvision.transforms.CenterCrop(size=9),
            # torchvision.transforms.Resize(size=size)
            # torchvision.transforms.RandomGrayscale(p=0.2),
            # RandomProjectionBands(n_band=8),
            # GroupPermuteBands(1)
